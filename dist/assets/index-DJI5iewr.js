...
`;const ki=document.getElementById("serverSwitcher"),Ti=document.getElementById("serverCurrent"),It=document.getElementById("serverCurrentIcon"),ht=document.getElementById("serverCurrentName"),Wt=document.getElementById("serverDropdown"),kt=document.getElementById("serverDropdownList"),Li=document.getElementById("serverDropdownCount"),ra=document.getElementById("profileAvatar"),Mt=document.getElementById("profileDisplayName"),Pt=document.getElementById("profileUsername"),ca=document.getElementById("ownerAvatar"),Ot=document.getElementById("ownerDisplayName"),_t=document.getElementById("ownerUsername"),Qe=document.getElementById("keyTimeCard"),Ye=document.getElementById("keyTimeCountdown"),Ze=document.getElementById("keyTimeExpiration");let Ee=null;document.getElementById("welcomeUsername");let he=[],K=localStorage.getItem("nebulaSelectedServerId")||"";function et(a){return String(a||"N").split(/\s+/).filter(Boolean).slice(0,2).map(e=>e.charAt(0)).join("").toUpperCase()}function Vt(a){if(!a){ht.textContent="Seleccioná un servidor",It.textContent="N";return}ht.textContent=a.name||"Servidor",a.icon?It.innerHTML=`
      <img
        src="${a.icon}"
        alt="${a.name}"
      >
    `:It.textContent=et(a.name)}function Jt(a){a?.id&&(K=String(a.id),localStorage.setItem("nebulaSelectedServerId",K),Vt(a),Wt.classList.remove("open"),window.dispatchEvent(new CustomEvent("nebula:server-changed",{detail:{server:a}})))}function bt(){if(Li.textContent=String(he.length),he.length===0){kt.innerHTML=`
      <div class="server-dropdown-empty">
    ...
    `,Vt(null);return}let a=he.find(e=>String(e.id)===String(K));a||(a=he[0],K=String(a.id),localStorage.setItem("nebulaSelectedServerId",K)),kt.innerHTML=he.map(e=>{const t=String(e.id)===String(K),i=e.icon?`<img
        ...
        `}).join(""),Vt(a),kt.querySelectorAll("[data-server-id]").forEach(e=>{e.addEventListener("click",()=>{const t=he.find(i=>String(i.id)===String(e.dataset.serverId));Jt(t),bt(),t?.botPresent&&Pe(t.id)})})}let x=null,Ut={name:"Nebula Bot",avatar:""};function Bi(a){const e=Math.max(0,Math.floor(a/1e3)),t=Math.floor(e/86400),i=Math.floor(e%86400/3600),n=Math.floor(e%3600/60),c=e%60;return`${t}d ${String(i).padStart(2,"0")}h ${String(n).padStart(2,"0")}m ${String(c).padStart(2,"0")}s`}function qe(){Ee&&(clearInterval(Ee),Ee=null),Qe&&(Qe.hidden=!0)}function Di(a){if(!Qe||!Ye||!Ze)return;if(Ee&&(clearInterval(Ee),Ee=null),Qe.hidden=!1,a.permanent===!0){Ye.textContent="Permanente",Ze.textContent="Sin vencimiento";return}const e=Number(a.expiresAt);if(!Number.isFinite(e)){qe();return}Ze.textContent=new Date(e).toLocaleString("es-AR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"});const t=()=>{const i=e-Date.now();if(i<=0){Ee&&(clearInterval(Ee),Ee=null),Ye.textContent="00d 00h 00m 00s",Ze.textContent="Key vencida",Sa();return}Ye.textContent=Bi(i)};t(),Ee=setInterval(t,1e3)}function Sa(){if(document.getElementById("expiredLicenseOverlay"))return;const a=document.createElement("div");a.id="expiredLicenseOverlay",a.innerHTML=`
  ...
  `,document.body.appendChild(a),document.getElementById("expiredLicenseReturnButton").addEventListener("click",()=>{a.remove(),x&&(x.hasAccess=!1),qe(),Xt(),window.history.replaceState({},"","/?view=access")})}async function xi(){try{const a=await fetch(`${O}/api/licenses/current`,{method:"GET",headers:{Accept:"application/json"},credentials:"include",cache:"no-store"}),e=await a.json();if(!a.ok||!e.success){qe();return}if(e.isOwner===!0){qe();return}if(e.license?.status==="expired"||e.license?.expiresAt&&Number(e.license.expiresAt)<=Date.now()){const t=new URLSearchParams(window.location.search).get("view");if(x&&(x.hasAccess=!1),t==="access"){qe(),document.getElementById("expiredLicenseOverlay")?.remove();return}Qe&&(Qe.hidden=!1),Ye&&(Ye.textContent="00d 00h 00m 00s"),Ze&&(Ze.textContent="Key vencida"),Sa();return}if(e.hasLicense!==!0||!e.license){qe();return}Di(e.license)}catch(a){console.error("No se pudo cargar el contador de la Key:",a),qe()}}async function Ni(){const a=new AbortController,e=setTimeout(()=>{a.abort()},1e4);try{const t=await fetch(`${O}/api/dashboard/session`,{method:"GET",headers:{Accept:"application/json"},credentials:"include",cache:"no-store",signal:a.signal}),i=await t.json();if(t.status===401||!i.authenticated){window.location.replace("/auth/dashboard");return}if(!t.ok||!i.success||!i.data)throw new Error(i.message||"No se pudo cargar la sesión.");const n=i.data;x=n.user||null;const c=x?.hasAccess===!0||x?.isOwner===!0;he=Array.isArray(n.guilds)?n.guilds:[],qt(x),bt(),await xi();const s=new URLSearchParams(window.location.search);s.get("view")==="access"||!c?(Xt(),window.history.replaceState({},"","/?view=access")):s.get("view")==="servers"?tt():(V.innerHTML=Aa,qt(x),bt(),Yt())}catch(t){if(console.error("Error cargando la sesión:",t),t.name==="AbortError"){Mt.textContent="Servidor sin respuesta",Pt.innerHTML="<i></i> Reintentá",Ot.textContent="Servidor sin respuesta",_t.textContent="Recargá la página",ht.textContent="Sin respuesta";return}Mt.textContent="Sesión no disponible",Pt.innerHTML="<i></i> Discord",Ot.textContent="Sesión no disponible",_t.textContent="@discord",ht.textContent="Sin conexión",setTimeout(()=>{window.location.replace("/auth/dashboard")},1500)}finally{clearTimeout(e)}}function qt(a){if(!a)return;const e=a.displayName||a.globalName||a.username||"Usuario",t=a.username||"discord";Mt.textContent=e,Pt.innerHTML=`
    <i></i>
    @${t}
  `,Ot.textContent=e,_t.textContent=`@${t}`;const i=document.getElementById("welcomeUsername"),n=document.getElementById("publicLicenses");if(n&&(n.style.display=a?.isOwner?"flex":"none"),i&&(i.textContent=e),a.avatar){const c=`
      <img
        ...
        alt="${e}"
      >
    `;ra.innerHTML=c,ca.innerHTML=c}else{const c=et(e);ra.textContent=c,ca.textContent=c}}function Xt(){document.body.classList.add("servers-selection-mode");const a=x?.displayName||x?.username||"Usuario",e=x?.username||"discord";V.innerHTML=`
  ...
  `;const t=document.getElementById("accessKeyInput"),i=document.getElementById("validateAccessKey"),n=document.getElementById("accessMessage");t?.addEventListener("input",()=>{t.value=t.value.toUpperCase().replace(/[^A-Z0-9-]/g,"")}),i?.addEventListener("click",async()=>{const c=t.value.trim().toUpperCase();if(!c){n.textContent="Ingresá una Key de acceso.";return}i.disabled=!0,i.textContent="Validando...",n.textContent="Comprobando la Key...";try{const s=await fetch(`${O}/api/licenses/activate`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},credentials:"include",body:JSON.stringify({key:c})}),p=await s.json();if(!s.ok||!p.success)throw new Error(p.message||"La Key no es válida.");n.textContent="Key activada correctamente. Entrando...",x&&(x.hasAccess=!0),setTimeout(()=>{window.location.replace("/?view=servers")},700)}catch(s){n.textContent=s.message||"No se pudo validar la Key.",i.disabled=!1,i.textContent="Validar Key"}})}function tt(){document.body.classList.add("servers-selection-mode");const e=he.map(t=>{const i=et(t.name),n=t.icon?`
              <img
                src="${t.icon}"
                alt="${t.name}"
              >
    ...
    <div class="dynamic-page session-servers-page">
      ...
      <section class="session-user-summary">
        ...
        <div>
          ...
          <strong>
            ${x?.displayName||x?.username||"Usuario"}
          </strong>
...
<section class="panel licenses-list-panel">
  ...
  >
    <div class="server-dropdown-empty">
      ...
      `;const a=document.getElementById("generateLicenseButton"),e=document.getElementById("deleteAllLicensesButton"),t=document.getElementById("licensesTable"),i=document.getElementById("licenseTotal"),n=document.getElementById("licenseActive"),c=document.getElementById("licenseUnused"),s=document.getElementById("licenseRevoked"),p=document.getElementById("licenseLogsChannel"),m=document.getElementById("saveLicenseSettings"),B=document.getElementById("licenseLogCreation"),$=document.getElementById("licenseLogActivation"),h=document.getElementById("licenseLogExpiration"),_=document.getElementById("licenseLogDeletion"),L=document.getElementById("licenseLogRevocation");async function Fe(){if(!p)return!1;if(!K)return p.innerHTML=`
      <option value="">
  ...
  `;try{const o=await fetch(`${O}/api/servers/${encodeURIComponent(K)}/text-channels`,{method:"GET",headers:{Accept:"application/json"},credentials:"include",cache:"no-store"}),u=await o.json();if(!o.ok||!u.success)throw new Error(u.message||"No se pudieron cargar los canales.");const y=Array.isArray(u.data)?u.data:Array.isArray(u.channels)?u.channels:[];return p.innerHTML=`
    ...
    `,p.disabled=!0,!1}}async function l(){if(!K||!p)return!1;try{const o=await fetch(`${O}/api/servers/${encodeURIComponent(K)}/license-settings`,{method:"GET",headers:{Accept:"application/json"},credentials:"include",cache:"no-store"}),u=await o.json();if(!o.ok||!u.success)throw new Error(u.message||"No se pudo cargar la configuración.");const y=u.data||{},g=String(y.logsChannelId||""),E=[...p.options].some(w=>String(w.value)===g);return p.value=E?g:"",B&&(B.checked=y.creation!==!1),$&&($.checked=y.activation!==!1),h&&(h.checked=y.expiration!==!1),_&&(_.checked=y.deletion!==!1),L&&(L.checked=y.revocation!==!1),!0}catch(o){return console.error("Error cargando configuración de licencias:",o),!1}}async function pe(){try{await Fe()&&await l()}catch(o){console.error("No se pudo inicializar la configuración de licencias:",o)}}window.addEventListener("nebula:server-changed",async()=>{document.getElementById("licenseLogsChannel")&&await pe()});async function ke(){if(!K){alert("Primero seleccioná un servidor.");return}if(!m)return;const o=m.textContent;m.disabled=!0,m.textContent="Guardando...";try{const u=await fetch(`${O}/api/servers/${encodeURIComponent(K)}/license-settings`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},credentials:"include",body:JSON.stringify({logsChannelId:p?.value||"",creation:B?.checked===!0,activation:$?.checked===!0,expiration:h?.checked===!0,deletion:_?.checked===!0,revocation:L?.checked===!0})}),y=await u.json();if(!u.ok||!y.success)throw new Error(y.message||"No se pudo guardar la configuración.");m.textContent="✓ Configuración guardada",setTimeout(()=>{m.textContent=o},1800)}catch(u){console.error("Error guardando configuración de licencias:",u),alert(u.message||"No se pudo guardar la configuración."),m.textContent=o}finally{m.disabled=!1}}m?.addEventListener("click",ke),pe();function oe(){document.querySelector(".license-config-overlay")?.remove(),document.body.insertAdjacentHTML("beforeend",`
  ...
  `}function Oe(o,u=!1){const y=o?.key||o?.licenseKey||o?.token||"KEY NO DISPONIBLE",g=o?.createdAt||o?.created||new Date().toISOString(),E=Number(o?.durationDays??o?.duration??0),w=Number(o?.durationHours??0),M=Number(o?.durationMinutes??0),z=Number(o?.durationSeconds??0),j=Number(o?.durationMilliseconds??E*864e5+w*36e5+M*6e4+z*1e3),N=o?.permanent===!0||E>=9999||String(o?.type||"").toLowerCase()==="permanente",J=String(o?.description||"Sin descripción").trim(),te=N?"Permanente":E>0?`${E} ${E===1?"día":"días"}`:o?.type||o?.plan||"Dashboard",ae=N?null:o?.expiresAt||o?.expirationDate||o?.expires||(j>0?new Date(new Date(g).getTime()+j).toISOString():null),C=Te(o);return N||ae&&`${ve(ae)}`,`
    <div
      class="license-generated-card ${u?"license-new-entry":""}"
      data-license-key="${D(y)}"
      data-license-duration="${D(String(E))}"
      data-license-permanent="${N?"true":"false"}"
  ...
  `}let Be=null;function Y(){document.querySelectorAll("[data-license-countdown]").forEach(u=>{const y=u.dataset.expiresAt,g=u.querySelector("[data-countdown-days]"),E=u.querySelector("[data-countdown-hours]"),w=u.querySelector("[data-countdown-minutes]"),M=u.querySelector("[data-countdown-seconds]"),z=/^\d+$/.test(String(y))?Number(y):new Date(y).getTime();if(!y||Number.isNaN(z)){u.innerHTML=`
      ...
      `;return}const j=z-Date.now();if(j<=0){u.innerHTML=`
        ...
        `;const C=u.closest(".license-generated-card");C&&!C.dataset.expired&&(C.dataset.expired="true",Ge(C.dataset.licenseKey));return}const N=Math.floor(j/864e5),J=Math.floor(j%864e5/36e5),te=Math.floor(j%36e5/6e4),ae=Math.floor(j%6e4/1e3);g&&(g.textContent=String(N).padStart(2,"0")),E&&(E.textContent=String(J).padStart(2,"0")),w&&(w.textContent=String(te).padStart(2,"0")),M&&(M.textContent=String(ae).padStart(2,"0"))})}async function Ge(o){try{await fetch(`/api/licenses/${encodeURIComponent(o)}`,{method:"DELETE"});const u=document.querySelector(`[data-license-key="${CSS.escape(o)}"]`);u&&u.remove(),ee()}catch(u){console.error("Error eliminando licencia:",u)}}function _e(){clearInterval(Be),Y(),Be=setInterval(Y,1e3)}function Z(){return`
  ...
  `;try{const o=await fetch(`${O}/api/licenses`,{method:"GET",credentials:"include",headers:{Accept:"application/json"},cache:"no-store"}),u=await o.json();if(!o.ok||!u.success)throw new Error(u.message||"No se pudieron cargar las licencias.");const y=Array.isArray(u.licenses)?u.licenses:[];if(y.length===0){t.innerHTML=q(),ee();return}t.innerHTML=`
    ...
    `,ee()}}}function ze(){t?.querySelector(".server-dropdown-empty")?.remove(),t?.querySelector(".licenses-empty-state")?.remove(),t?.querySelector(".license-generation-error")?.remove()}function ee(){if(!t)return;const o=[...t.querySelectorAll(".license-generated-card")],u=o.length,y=o.filter(w=>w.querySelector(".license-status.active, .license-status.used")).length,g=o.filter(w=>w.querySelector(".license-status.available")).length,E=o.filter(w=>w.querySelector(".license-status.revoked")).length;i&&(i.textContent=String(u)),n&&(n.textContent=String(y)),c&&(c.textContent=String(g)),s&&(s.textContent=String(E))}function it(){t&&(t.querySelector(".license-generating-overlay")?.remove(),document.body.insertAdjacentHTML("beforeend",`
...
${w}
${M}
Estado: ${E}`)}function R(o){const u=o?.dataset.licenseKey||"",y=prompt("Editar el nombre visible de la Key:",u);if(!y||y.trim()===u)return;const g=y.trim().toUpperCase();o.dataset.licenseKey=g;const E=o.querySelector(".license-generated-info > strong"),w=o.querySelector(".license-action-btn.copy");E&&(E.textContent=g,E.title=g),w&&(w.dataset.licenseKey=g)}async function U(o){const u=o?.dataset.licenseKey||"";if(!u){window.alert("No se pudo obtener la Key.");return}if(window.confirm(`¿Querés eliminar ${u} definitivamente?`))try{const g=await fetch(`${O}/api/licenses/${encodeURIComponent(u)}`,{method:"DELETE",headers:{Accept:"application/json"},credentials:"include"}),E=await g.json();if(!g.ok||!E.success)throw new Error(E.message||"No se pudo eliminar la licencia.");o.style.opacity="0",o.style.transform="translateX(20px)",setTimeout(()=>{o.remove(),ee(),t.querySelectorAll(".license-generated-card").length===0&&(t.innerHTML=q())},220)}catch(g){console.error("Error eliminando licencia:",g),window.alert(g.message||"No se pudo eliminar la licencia.")}}t?.addEventListener("click",o=>{const u=o.target.closest("[data-license-action]");if(!u)return;const y=u.closest(".license-generated-card"),g=u.dataset.licenseAction;if(g==="copy"){b(u);return}if(g==="view"){A(y);return}if(g==="edit"){R(y);return}g==="delete"&&U(y)}),t&&De(),je.off("licenses:update"),je.on("licenses:update",async()=>{document.getElementById("licensesTable")&&(console.log("Cambio de licencias recibido en tiempo real."),await De())});async function re(o){const u=Number(o.days||0),y=Number(o.hours||0),g=Number(o.minutes||0),E=Number(o.seconds||0),w=o.permanent===!0||u>=9999,M=u,z=a.innerHTML;a.disabled=!0,a.innerHTML="Generando Key...",it();try{ye("Validando datos..."),await fe(350),ye("Generando código único...");const j=await fetch(`${O}/api/licenses/generate`,{method:"POST",credentials:"include",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({guildId:K,days:u,hours:y,minutes:g,seconds:E,durationDays:u,duration:u,permanent:w,description:o.description})});let N;try{N=await j.json()}catch{throw new Error("El servidor devolvió una respuesta inválida.")}if(!j.ok||!N.success)throw new Error(N.message||"No se pudo generar la Key.");const J=Array.isArray(N.licenses)?N.licenses[0]:N.license||N.data||null,te=J?.key||J?.licenseKey||J?.token,ae=J?.createdAt||new Date().toISOString(),C=w?null:J?.expiresAt||J?.expirationDate||J?.expires||(o.durationMilliseconds?new Date(new Date(ae).getTime()+o.durationMilliseconds).toISOString():null);if(!te)throw new Error("El servidor no devolvió la Key.");ye("Guardando en el sistema..."),await fe(450),Ke(),await fe(650),xe(),ze(),me(),t.insertAdjacentHTML("beforeend",Oe({...J,key:te,duration:u,durationDays:u,durationHours:y,durationMinutes:g,durationSeconds:E,durationMilliseconds:o.durationMilliseconds,permanent:w,description:o.description,type:w?"Permanente":`${M} ${M===1?"día":"días"}`,createdAt:ae,expiresAt:C},!0)),_e(),ee(),t.scrollTo({top:t.scrollHeight,behavior:"smooth"})}catch(j){console.error("Error generando licencia:",j),xe(),alert(j.message||"No se pudo generar la licencia.")}finally{a.disabled=!1,a.innerHTML=z}}a?.addEventListener("click",()=>{oe()}),e?.addEventListener("click",()=>{Q()})});document.getElementById("publicSupport")?.addEventListener("click",()=>{Me(),document.body.classList.add("servers-selection-mode"),V.innerHTML=`
        <div class="dynamic-page">
          <section class="section-header">
            <div>
              <span>
...
`;Ni();window.addEventListener("nebula:server-changed",async()=>{document.getElementById("licenseLogsChannel")&&await initializeLicenseSettings()});const Oi={Servidores:{eyebrow:"SERVIDORES",title:"Tus servidores",description:"Seleccioná un servidor para administrar sus funciones, permisos y configuración.",action:"＋ Agregar servidor",type:"servers"},Invitaciones:{eyebrow:"SERVIDORES",title:"Invitaciones",description:"Creá, controlá y eliminá enlaces de invitación para tus comunidades.",action:"＋ Nueva invitación",type:"invitations"},Miembros:{eyebrow:"COMUNIDAD",title:"Miembros",description:"Buscá usuarios, revisá sus roles y consultá su actividad reciente.",action:"Exportar lista",type:"members"},Roles:{eyebrow:"COMUNIDAD",title:"Roles",description:"Organizá la jerarquía y los permisos de cada rol del servidor.",action:"＋ Crear rol",type:"roles"},Canales:{eyebrow:"ESTRUCTURA",title:"Canales",description:"Administrá canales de texto, voz, categorías y permisos.",action:"＋ Crear canal",type:"channels"},"Logs del servidor":{eyebrow:"REGISTROS",title:"Logs del servidor",description:"Consultá eventos, mensajes eliminados y movimientos administrativos.",action:"Exportar logs",type:"logs"},Comandos:{eyebrow:"COMANDOS",title:"Comandos",description:"Activá, desactivá y personalizá las funciones principales del bot.",action:"＋ Crear comando",type:"commands"},"Slash Commands":{eyebrow:"COMANDOS",title:"Slash Commands",description:"Gestioná los comandos que aparecen al escribir / dentro de Discord.",action:"Sincronizar",type:"slash"},Mensajes:{eyebrow:"MENSAJES",title:"Mensajes automáticos",description:"Diseñá mensajes de bienvenida, despedida, anuncios y verificaciones.",action:"＋ Nuevo mensaje",type:"messages"},"Auto Respuestas":{eyebrow:"AUTOMATIZACIÓN",title:"Auto Respuestas",description:"Configurá respuestas automáticas mediante palabras o frases clave.",action:"＋ Nueva respuesta",type:"responses"},Moderación:{eyebrow:"SEGURIDAD",title:"Centro de moderación",description:"Accedé rápidamente a todas las herramientas de control del servidor.",action:"Abrir historial",type:"moderation"},Advertencias:{eyebrow:"MODERACIÓN",title:"Advertencias",description:"Registrá faltas, motivos, responsables y vencimientos.",action:"＋ Advertir usuario",type:"warnings"},Baneos:{eyebrow:"MODERACIÓN",title:"Baneos",description:"Administrá usuarios bloqueados y revisá los motivos de cada sanción.",action:"＋ Banear usuario",type:"bans"},Muteos:{eyebrow:"MODERACIÓN",title:"Muteos",description:"Aplicá silenciamientos temporales y revisá los que siguen activos.",action:"＋ Mutear usuario",type:"mutes"},"Anti Raid":{eyebrow:"PROTECCIÓN",title:"Anti Raid",description:"Protegé el servidor contra ingresos masivos y comportamientos sospechosos.",action:"Activar protección",type:"antiraid"},"Auto Mod":{eyebrow:"PROTECCIÓN",title:"Auto Mod",description:"Filtrá spam, enlaces, menciones masivas y palabras prohibidas.",action:"Guardar reglas",type:"automod"},Logs:{eyebrow:"SISTEMA",title:"Logs generales",description:"Revisá todas las acciones realizadas por el bot y el dashboard.",action:"Descargar",type:"logs"},Auditoría:{eyebrow:"SISTEMA",title:"Auditoría",description:"Identificá quién realizó cada cambio dentro del panel.",action:"Filtrar actividad",type:"audit"},"Tareas programadas":{eyebrow:"AUTOMATIZACIÓN",title:"Tareas programadas",description:"Programá anuncios, limpiezas, backups y acciones automáticas.",action:"＋ Programar tarea",type:"tasks"},Backups:{eyebrow:"SISTEMA",title:"Backups",description:"Guardá y restaurá configuraciones del servidor de forma segura.",action:"Crear backup",type:"backups"},Webhooks:{eyebrow:"INTEGRACIONES",title:"Webhooks",description:"Conectá servicios externos para recibir y enviar información.",action:"＋ Crear webhook",type:"webhooks"},Configuración:{eyebrow:"AJUSTES",title:"Configuración general",description:"Definí la identidad, el estado y el comportamiento principal del bot.",action:"Guardar cambios",type:"settings"},Personalización:{eyebrow:"APARIENCIA",title:"Personalización",description:"Cambiá colores, textos, imágenes y estilo visual del panel.",action:"Aplicar diseño",type:"customize"},"Variables de entorno":{eyebrow:"DESARROLLO",title:"Variables de entorno",description:"Prepará las variables que usaremos luego en Render sin mostrar secretos reales.",action:"Guardar variables",type:"environment"},Tokens:{eyebrow:"SEGURIDAD",title:"Tokens",description:"Visualizá el estado de las credenciales sin exponer su contenido.",action:"Actualizar token",type:"tokens"},Permisos:{eyebrow:"SEGURIDAD",title:"Permisos del bot",description:"Revisá qué acciones puede realizar el bot dentro de Discord.",action:"Guardar permisos",type:"permissions"},Integraciones:{eyebrow:"CONEXIONES",title:"Integraciones",description:"Prepará conexiones con Discord, Render, GitHub y otros servicios.",action:"＋ Conectar servicio",type:"integrations"}};function _i(a){return`
  ...
  `;try{const e=await fetch(`${O}/api/servers/${a}`),t=await e.json();if(!e.ok||!t.success)throw new Error(t.message||"No se pudo cargar el servidor");ji(t.data)}catch(e){console.error("Error cargando servidor:",e),V.innerHTML=`
      <div class="dynamic-page">
  ...
  `,_=({id:r,title:d,description:v,enabled:f,logChannelId:T="",channels:G="",showLogChannel:F=!0,icon:X="◈",extraContent:ie="",cardClass:P=""})=>{const se=String(G).replace(new RegExp(`value="${String(T)}"`,"g"),`value="${String(T)}" selected`);return`
    <article
      class="verify-security-card ${P}"
  ...
  `},L=(r,d,v)=>h(`verifyLog_${r}`,d,v,!!c[r]);V.innerHTML=`
...
>
  ...
  <article class="section-panel verify-security-section">
    ...
    <div class="verify-security-list">
      ...
      ${_({id:"verifyDetectVpn",title:"Detectar VPN",description:"Marca conexiones que utilizan redes privadas virtuales.",enabled:p.detectVpn,logChannelId:p.detectVpnLogChannelId,channels:m,icon:"◉"})}

      ${_({id:"verifyDetectProxy",title:"Detectar Proxy",description:"Bloquea conexiones realizadas mediante servidores Proxy.",enabled:p.detectProxy,logChannelId:p.detectProxyLogChannelId,channels:m,icon:"⌁"})}

      ${_({id:"verifyDetectTor",title:"Detectar Tor",description:"Detecta conexiones realizadas desde la red anónima Tor.",enabled:p.detectTor,logChannelId:p.detectTorLogChannelId,channels:m,icon:"◎"})}

      ${_({id:"verifyBlockHosting",title:"Bloquear Hosting",description:"Impide verificaciones desde servidores, datacenters o VPS.",enabled:p.blockHosting,logChannelId:p.blockHostingLogChannelId,channels:m,icon:"▣"})}

      ${_({id:"verifyDetectAltAccounts",title:"Detectar Multicuentas",description:"Detecta usuarios que podrían utilizar cuentas alternativas.",enabled:p.detectAltAccounts,logChannelId:p.detectAltAccountsLogChannelId,channels:m,icon:"♧"})}

      ${_({id:"verifyMinimumAgeEnabled",title:"Edad mínima de la cuenta",description:"Bloquea cuentas de Discord creadas recientemente.",enabled:p.minimumAccountAgeEnabled,logChannelId:p.minimumAccountAgeLogChannelId,channels:m,icon:"◷",cardClass:"verify-security-age-card",extraContent:`
      ...
      ${_({id:"verifyBlockWithoutAvatar",title:"Bloquear sin Avatar",description:"Impide verificar usuarios que no tengan foto de perfil.",enabled:p.blockWithoutAvatar,logChannelId:p.blockWithoutAvatarLogChannelId,channels:m,icon:"●"})}

      ${_({id:"verifyBlockWithoutBanner",title:"Bloquear sin Banner",description:"Impide verificar usuarios que no tengan banner configurado.",enabled:p.blockWithoutBanner,logChannelId:p.blockWithoutBannerLogChannelId,channels:m,icon:"▰"})}

      ${_({id:"verifyAllowReverification",title:"Permitir Re-verificación",description:"Permite que un usuario vuelva a realizar el proceso.",enabled:p.allowReverification,showLogChannel:!1,icon:"↻",cardClass:"verify-security-simple-card"})}

      ${_({id:"verifyNotifySecurityFailure",title:"Notificar fallo de seguridad",description:"Envía un aviso cuando una verificación es rechazada.",enabled:p.notifySecurityFailure,showLogChannel:!1,icon:"!",cardClass:"verify-security-simple-card"})}
...
`}[Ke,xe,A,R,U].forEach(r=>{r?.addEventListener("input",M)}),fe?.addEventListener("input",M),b?.addEventListener("input",()=>{const r=b.value.trim();w(r)&&fe&&(fe.value=r,M())}),M(),l("backToServerPanel")?.addEventListener("click",()=>{Pe(a.id)}),document.querySelectorAll(".verify-tab").forEach(r=>{r.addEventListener("click",()=>{document.querySelectorAll(".verify-tab").forEach(d=>d.classList.remove("active")),document.querySelectorAll(".verify-tab-panel").forEach(d=>d.classList.remove("active")),r.classList.add("active"),document.querySelector(`[data-verify-panel="${r.dataset.verifyTab}"]`)?.classList.add("active")})}),l("verifyEnabled")?.addEventListener("change",r=>{const d=l("verifySystemStatus");d.textContent=r.target.checked?"ACTIVADO":"DESACTIVADO",d.classList.toggle("enabled",r.target.checked),d.classList.toggle("disabled",!r.target.checked)}),l("verifyLogs")?.addEventListener("change",r=>{l("verifyLogsSettings")?.classList.toggle("visible",!!r.target.value)});const z=(r,d)=>{const v=l(r),f=l(d);v?.addEventListener("input",()=>{f.value=v.value}),f?.addEventListener("input",()=>{/^#[0-9A-F]{6}$/i.test(f.value)&&(v.value=f.value)})};z("verifyEmbedColor","verifyEmbedColorText"),z("verifyLogColor","verifyLogColorText");function j(){return{splitImageUrl:document.getElementById("verifySplitImageUrl")?.value.trim()||"",splitImagePosition:document.getElementById("verifySplitImagePosition")?.value||"left",splitImageFit:document.getElementById("verifySplitImageFit")?.value||"cover",splitImageDarkness:Number(document.getElementById("verifySplitImageDarkness")?.value||45),splitImageWidth:Number(document.getElementById("verifySplitImageWidth")?.value||48),splitShowImage:document.getElementById("verifySplitShowImage")?.checked!==!1,splitShowDate:document.getElementById("verifySplitShowDate")?.checked!==!1,splitShowAccess:document.getElementById("verifySplitShowAccess")?.checked!==!1,cardDesign:document.querySelector('input[name="verificationCardDesign"]:checked')?.value||"classic",terminalTitle:document.getElementById("verifyTerminalTitle")?.value.trim()||"NEBULA SECURITY TERMINAL",terminalPrefix:document.getElementById("verifyTerminalPrefix")?.value.trim()||">",terminalStatusText:document.getElementById("verifyTerminalStatusText")?.value.trim()||"Sistema preparado",terminalBackgroundColor:document.getElementById("verifyTerminalBackgroundColorText")?.value||"#020703",terminalTextColor:document.getElementById("verifyTerminalTextColorText")?.value||"#d9ffe0",terminalAccentColor:document.getElementById("verifyTerminalAccentColorText")?.value||"#22c55e",terminalBorderColor:document.getElementById("verifyTerminalBorderColorText")?.value||"#14532d",terminalShowCursor:!!document.getElementById("verifyTerminalShowCursor")?.checked,terminalShowLines:!!document.getElementById("verifyTerminalShowLines")?.checked,terminalShowServer:!!document.getElementById("verifyTerminalShowServer")?.checked,terminalShowRole:!!document.getElementById("verifyTerminalShowRole")?.checked,terminalGlow:Number(document.getElementById("verifyTerminalGlow")?.value||25),terminalRadius:Number(document.getElementById("verifyTerminalRadius")?.value||10),pageName:document.getElementById("verifyPageName")?.value.trim()||"",pageDescription:document.getElementById("verifyPageDescription")?.value.trim()||"",logoUrl:document.getElementById("verifyLogoUrl")?.value.trim()||"",backgroundUrl:document.getElementById("verifyBackgroundUrl")?.value.trim()||"",primaryColor:document.getElementById("verifyPrimaryColorText")?.value||"#8b5cf6",secondaryColor:document.getElementById("verifySecondaryColorText")?.value||"#6d28d9",buttonColor:document.getElementById("verifyButtonColorText")?.value||"#7c3aed",textColor:document.getElementById("verifyTextColorText")?.value||"#ffffff",cardColor:document.getElementById("verifyCardColorText")?.value||"#0f0f1a",backgroundSolidColor:document.getElementById("verifyBackgroundSolidColorText")?.value||"#05050a",gradientStart:document.getElementById("verifyGradientStartText")?.value||"#05050a",gradientEnd:document.getElementById("verifyGradientEndText")?.value||"#160c2b",backgroundType:document.getElementById("verifyBackgroundType")?.value||"space",spaceBackground:!!document.getElementById("verifySpaceBackground")?.checked,animationsEnabled:!!document.getElementById("verifyAnimationsEnabled")?.checked,particlesEnabled:!!document.getElementById("verifyParticlesEnabled")?.checked,glowEnabled:!!document.getElementById("verifyGlowEnabled")?.checked,fadeEnabled:!!document.getElementById("verifyFadeEnabled")?.checked,hoverEnabled:!!document.getElementById("verifyHoverEnabled")?.checked,cursorGlowEnabled:!!document.getElementById("verifyCursorGlowEnabled")?.checked,buttonAnimationEnabled:!!document.getElementById("verifyButtonAnimationEnabled")?.checked,logoAnimationEnabled:!!document.getElementById("verifyLogoAnimationEnabled")?.checked,particleCount:Number(document.getElementById("verifyParticleCount")?.value||100),glowIntensity:Number(document.getElementById("verifyGlowIntensity")?.value||80),cardBlur:Number(document.getElementById("verifyCardBlur")?.value||18),cardOpacity:Number(document.getElementById("verifyCardOpacity")?.value||88),cardRadius:Number(document.getElementById("verifyCardRadius")?.value||24),cardShadow:Number(document.getElementById("verifyCardShadow")?.value||80),verifyButtonText:document.getElementById("verifyAppearanceButtonText")?.value.trim()||"Verificar con Discord",verifyButtonIcon:document.getElementById("verifyButtonIcon")?.value||"discord",verifyButtonSize:document.getElementById("verifyButtonSize")?.value||"large",verifyButtonRadius:Number(document.getElementById("verifyButtonRadius")?.value||14),verificationSound:!!document.getElementById("verifyVerificationSound")?.checked,errorSound:!!document.getElementById("verifyErrorSound")?.checked,openingSound:!!document.getElementById("verifyOpeningSound")?.checked,soundVolume:Number(document.getElementById("verifySoundVolume")?.value||50)}}function N(){const r=j(),d=document.getElementById("verifyAppearanceFrame");d?.contentWindow&&d.contentWindow.postMessage({type:"nebula-appearance-preview",appearance:r},window.location.origin)}document.getElementById("verifyAppearanceFrame")?.addEventListener("load",()=>{setTimeout(N,150)});function te(){const r=document.querySelector('input[name="verificationCardDesign"]:checked')?.value||"classic";document.querySelectorAll("[data-design-settings]").forEach(f=>{f.classList.toggle("active",f.dataset.designSettings===r)});const d={classic:["identity","palette","background","animations","card","button","sounds"],split:["identity","palette","background","button"],terminal:["identity","background","button"]},v=d[r]||d.classic;document.querySelectorAll("[data-appearance-section]").forEach(f=>{const T=f.dataset.appearanceSection;f.hidden=!v.includes(T)})}document.querySelectorAll('input[name="verificationCardDesign"]').forEach(r=>{r.addEventListener("change",()=>{te(),N()})}),te(),["verifyPageName","verifyPageDescription","verifyLogoUrl","verifyBackgroundUrl","verifyPrimaryColor","verifyPrimaryColorText","verifySecondaryColor","verifySecondaryColorText","verifyButtonColor","verifyButtonColorText","verifyTextColor","verifyTextColorText","verifyCardColor","verifyCardColorText","verifyBackgroundSolidColor","verifyBackgroundSolidColorText","verifyGradientStart","verifyGradientStartText","verifyGradientEnd","verifyGradientEndText","verifyBackgroundType","verifySpaceBackground","verifyParticlesEnabled","verifyAnimationsEnabled","verifyGlowEnabled","verifyFadeEnabled","verifyHoverEnabled","verifyCursorGlowEnabled","verifyButtonAnimationEnabled","verifyLogoAnimationEnabled","verifyParticleCount","verifyGlowIntensity","verifyCardBlur","verifyCardOpacity","verifyCardRadius","verifyCardShadow","verifyAppearanceButtonText","verifyButtonIcon","verifyButtonSize","verifyButtonRadius","verifyOpeningSound","verifyVerificationSound","verifyErrorSound","verifySoundVolume","verifySplitImageUrl","verifySplitImagePosition","verifySplitImageFit","verifySplitImageDarkness","verifySplitImageWidth","verifySplitShowImage","verifySplitShowDate","verifySplitShowAccess","verifyTerminalTitle","verifyTerminalPrefix","verifyTerminalStatusText","verifyTerminalBackgroundColor","verifyTerminalBackgroundColorText","verifyTerminalTextColor","verifyTerminalTextColorText","verifyTerminalAccentColor","verifyTerminalAccentColorText","verifyTerminalBorderColor","verifyTerminalBorderColorText","verifyTerminalShowCursor","verifyTerminalShowLines","verifyTerminalShowServer","verifyTerminalShowRole","verifyTerminalGlow","verifyTerminalRadius"].forEach(r=>{const d=document.getElementById(r);if(!d)return;const v=d.type==="checkbox"||d.tagName==="SELECT"?"change":"input";d.addEventListener(v,N)}),[["verifyPrimaryColor","verifyPrimaryColorText"],["verifySecondaryColor","verifySecondaryColorText"],["verifyButtonColor","verifyButtonColorText"],["verifyTextColor","verifyTextColorText"],["verifyCardColor","verifyCardColorText"],["verifyBackgroundSolidColor","verifyBackgroundSolidColorText"],["verifyGradientStart","verifyGradientStartText"],["verifyGradientEnd","verifyGradientEndText"],["verifyTerminalBackgroundColor","verifyTerminalBackgroundColorText"],["verifyTerminalTextColor","verifyTerminalTextColorText"],["verifyTerminalAccentColor","verifyTerminalAccentColorText"],["verifyTerminalBorderColor","verifyTerminalBorderColorText"]].forEach(([r,d])=>{const v=document.getElementById(r),f=document.getElementById(d);v?.addEventListener("input",()=>{f.value=v.value,N()}),f?.addEventListener("input",()=>{const T=f.value.trim();/^#[0-9a-f]{6}$/i.test(T)&&(v.value=T,N())})}),document.querySelectorAll(".appearance-range-control input[type='range']").forEach(r=>{const d=document.getElementById(`${r.id}Value`),v=()=>{d&&(d.textContent=`${r.value}${r.dataset.suffix||""}`),N()};r.addEventListener("input",v),v()}),N(),l("saveVerification")?.addEventListener("click",async()=>{const r=l("saveVerification"),d=De();if(!d.verificationChannelId){Y("Falta el canal","Seleccioná el canal de verificación.");return}if(!d.verifiedRoleId){Y("Falta el rol","Seleccioná el rol que recibirá el usuario.");return}r.disabled=!0,r.textContent="Guardando...";try{const v=await fetch(`${O}/api/servers/${a.id}/verification`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)}),f=await v.json();if(!v.ok||!f.success)throw new Error(f.message||"No se pudo guardar.");Y("Configuración guardada","Todos los cambios quedaron guardados.")}catch(v){Y("Error al guardar",v.message)}finally{r.disabled=!1,r.textContent="Guardar configuración"}}),l("sendVerificationPanel")?.addEventListener("click",async()=>{const r=l("sendVerificationPanel");l("saveVerification").click(),await new Promise(v=>setTimeout(v,700)),r.disabled=!0,r.textContent="Enviando...";try{const v=await fetch(`${O}/api/servers/${a.id}/verification/send`,{method:"POST"}),f=await v.json();if(!v.ok||!f.success)throw new Error(f.message||"No se pudo enviar el panel.");Y("Panel enviado","Revisá el canal de verificación en Discord.")}catch(v){Y("Error al enviar",v.message)}finally{r.disabled=!1,r.textContent="Enviar panel a Discord"}})}async function Gi(a){V.innerHTML=`
  ...
  `;try{const[e,t]=await Promise.all([fetch(`${O}/api/servers/${a.id}/text-channels`),fetch(`${O}/api/servers/${a.id}/welcome`)]),i=await e.json(),n=await t.json();if(!i.success||!n.success)throw new Error("No se pudo cargar la configuración");zi(a,i.data,n.data)}catch(e){V.innerHTML=`
      <div class="dynamic-page">
        <div class="servers-error">
          <div class="servers-error-icon">!</div>
...
>
  ...
  `;const n=document.getElementById("welcomeEnabled"),c=document.getElementById("welcomeChannel"),s=document.getElementById("welcomeTitle"),p=document.getElementById("welcomeMessage"),m=document.getElementById("welcomeColor"),B=document.getElementById("welcomeColorText"),$=document.getElementById("welcomeShowAvatar"),h=document.getElementById("welcomeDmEnabled"),_=document.getElementById("welcomeDmFields"),L=document.getElementById("welcomeDmTitle"),Fe=document.getElementById("welcomeDmMessage"),l=document.getElementById("welcomeDmColor"),pe=document.getElementById("welcomeDmColorText"),ke=document.getElementById("welcomeDmShowAvatar"),oe=document.getElementById("welcomeDmPreviewSection"),W=Ut?.name||"Nebula Bot",D=x||{},ve=D.displayName||D.globalName||D.username||"Usuario",Te=D.avatar||"",Le=document.getElementById("welcomePreviewUserAvatar"),Se=document.getElementById("welcomeDmPreviewAvatar"),He=Te?`
      <img
        src="${Te}"
        alt="${ve}"
      >
    ...
    `;Le&&(Le.innerHTML=He),Se&&(Se.innerHTML=He);const Oe=Ut?.avatar||"",Be=document.getElementById("welcomePreviewBotName"),Y=document.getElementById("welcomeDmPreviewBotName"),Ge=document.getElementById("welcomePreviewBotAvatar"),_e=document.getElementById("welcomeDmPreviewBotAvatar");Be&&(Be.textContent=W),Y&&(Y.textContent=W);const Z=Oe?`
      <img
        src="${Oe}"
        alt="${W}"
      >
    `:W.charAt(0).toUpperCase();Ge&&(Ge.innerHTML=Z),_e&&(_e.innerHTML=Z);function q(){const b=pt(s.value,a),A=pt(p.value,a);document.getElementById("welcomePreviewTitle").textContent=b,document.getElementById("welcomePreviewMessage").textContent=A,document.getElementById("welcomePreviewEmbed").style.borderLeftColor=m.value;const R=document.getElementById("welcomePreviewUserAvatar");R&&(R.style.display=$.checked?"block":"none");const U=pt(L.value,a),re=pt(Fe.value,a);document.getElementById("welcomeDmPreviewTitle").textContent=U,document.getElementById("welcomeDmPreviewMessage").textContent=re,document.getElementById("welcomeDmPreviewEmbed").style.borderLeftColor=l.value;const o=document.getElementById("welcomeDmPreviewAvatar");o&&(o.style.display=ke.checked?"block":"none")}m.addEventListener("input",()=>{B.value=m.value,q()}),B.addEventListener("input",()=>{/^#[0-9A-F]{6}$/i.test(B.value)&&(m.value=B.value,q())}),s.addEventListener("input",q),p.addEventListener("input",q),h.addEventListener("change",()=>{_.classList.toggle("visible",h.checked),oe.classList.toggle("visible",h.checked),q()}),$.addEventListener("change",q),ke.addEventListener("change",q),L.addEventListener("input",q),Fe.addEventListener("input",q),l.addEventListener("input",()=>{pe.value=l.value,q()}),pe.addEventListener("input",()=>{/^#[0-9A-F]{6}$/i.test(pe.value)&&(l.value=pe.value,q())}),document.querySelectorAll("[data-variable]").forEach(b=>{b.addEventListener("click",()=>{const A=b.dataset.variable,R=p.selectionStart,U=p.selectionEnd;p.value=p.value.slice(0,R)+A+p.value.slice(U),p.focus(),p.selectionStart=p.selectionEnd=R+A.length,q()})});const Q=document.getElementById("welcomeVariablesModal"),be=document.getElementById("welcomeVariablesSearch"),me=document.getElementById("welcomeVariablesModalList"),De=document.getElementById("welcomeVariablesModalEmpty"),ze=document.querySelectorAll("[data-welcome-variable-category]");let ee="user";const it=[{key:"{user}",label:"Nombre corto del usuario",category:"user"},{key:"{mention}",label:"Mención del usuario",category:"user"},{key:"{username}",label:"Nombre de usuario",category:"user"},{key:"{displayname}",label:"Nombre visible",category:"user"},{key:"{userid}",label:"ID del usuario",category:"user"},{key:"{joindate}",label:"Fecha de ingreso",category:"user"},{key:"{server}",label:"Nombre del servidor",category:"server"},{key:"{serverid}",label:"ID del servidor",category:"server"},{key:"{members}",label:"Cantidad de miembros",category:"server"},{key:"{membercount}",label:"Cantidad de miembros",category:"server"},{key:"{date}",label:"Fecha actual",category:"date"},{key:"{time}",label:"Hora actual",category:"date"}];function ye(){if(!me)return;const b=String(be?.value||"").trim().toLowerCase(),A=it.filter(U=>{const re=U.category===ee,o=`${U.key} ${U.label}`.toLowerCase(),u=!b||o.includes(b);return re&&u});me.innerHTML=A.map(U=>`
          <button
            class="verify-variable-button"
            data-copy-welcome-variable="${U.key}"
            type="button"
          >
            <code>
              ${U.key}
