...
`;const Ei=document.getElementById("serverSwitcher"),wi=document.getElementById("serverCurrent"),Et=document.getElementById("serverCurrentIcon"),vt=document.getElementById("serverCurrentName"),Ft=document.getElementById("serverDropdown"),wt=document.getElementById("serverDropdownList"),Ci=document.getElementById("serverDropdownCount"),ta=document.getElementById("profileAvatar"),Dt=document.getElementById("profileDisplayName"),xt=document.getElementById("profileUsername"),aa=document.getElementById("ownerAvatar"),Nt=document.getElementById("ownerDisplayName"),$t=document.getElementById("ownerUsername");document.getElementById("welcomeUsername");let he=[],K=localStorage.getItem("nebulaSelectedServerId")||"";function We(a){return String(a||"N").split(/\s+/).filter(Boolean).slice(0,2).map(e=>e.charAt(0)).join("").toUpperCase()}function Rt(a){if(!a){vt.textContent="Seleccioná un servidor",Et.textContent="N";return}vt.textContent=a.name||"Servidor",a.icon?Et.innerHTML=`
  ...
  `,Nt.textContent=e,$t.textContent=`@${t}`;const i=document.getElementById("welcomeUsername"),n=document.getElementById("publicLicenses");if(n&&(n.style.display=a?.isOwner?"flex":"none"),i&&(i.textContent=e),a.avatar){const c=`
      <img
        ...
        alt="${e}"
      >
    `;ta.innerHTML=c,aa.innerHTML=c}else{const c=We(e);ta.textContent=c,aa.textContent=c}}function ga(){document.body.classList.add("servers-selection-mode");const a=U?.displayName||U?.username||"Usuario",e=U?.username||"discord";_.innerHTML=`
...
<section class="panel licenses-list-panel">
  ...
  `;try{const o=await fetch(`${O}/api/servers/${encodeURIComponent(K)}/text-channels`,{method:"GET",headers:{Accept:"application/json"},credentials:"include",cache:"no-store"}),u=await o.json();if(!o.ok||!u.success)throw new Error(u.message||"No se pudieron cargar los canales.");const y=Array.isArray(u.data)?u.data:Array.isArray(u.channels)?u.channels:[];return p.innerHTML=`
    ...
    `,p.disabled=!0,!1}}async function l(){if(!K||!p)return!1;try{const o=await fetch(`${O}/api/servers/${encodeURIComponent(K)}/license-settings`,{method:"GET",headers:{Accept:"application/json"},credentials:"include",cache:"no-store"}),u=await o.json();if(!o.ok||!u.success)throw new Error(u.message||"No se pudo cargar la configuración.");const y=u.data||{},g=String(y.logsChannelId||""),E=[...p.options].some(w=>String(w.value)===g);return p.value=E?g:"",B&&(B.checked=y.creation!==!1),N&&(N.checked=y.activation!==!1),h&&(h.checked=y.expiration!==!1),M&&(M.checked=y.deletion!==!1),L&&(L.checked=y.revocation!==!1),!0}catch(o){return console.error("Error cargando configuración de licencias:",o),!1}}async function pe(){try{await Ue()&&await l()}catch(o){console.error("No se pudo inicializar la configuración de licencias:",o)}}window.addEventListener("nebula:server-changed",async()=>{document.getElementById("licenseLogsChannel")&&await pe()});async function Ie(){if(!K){alert("Primero seleccioná un servidor.");return}if(!m)return;const o=m.textContent;m.disabled=!0,m.textContent="Guardando...";try{const u=await fetch(`${O}/api/servers/${encodeURIComponent(K)}/license-settings`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},credentials:"include",body:JSON.stringify({logsChannelId:p?.value||"",creation:B?.checked===!0,activation:N?.checked===!0,expiration:h?.checked===!0,deletion:M?.checked===!0,revocation:L?.checked===!0})}),y=await u.json();if(!u.ok||!y.success)throw new Error(y.message||"No se pudo guardar la configuración.");m.textContent="✓ Configuración guardada",setTimeout(()=>{m.textContent=o},1800)}catch(u){console.error("Error guardando configuración de licencias:",u),alert(u.message||"No se pudo guardar la configuración."),m.textContent=o}finally{m.disabled=!1}}m?.addEventListener("click",Ie),pe();function oe(){document.querySelector(".license-config-overlay")?.remove(),document.body.insertAdjacentHTML("beforeend",`
  ...
  `}let Le=null;function Y(){document.querySelectorAll("[data-license-countdown]").forEach(u=>{const y=u.dataset.expiresAt,g=u.querySelector("[data-countdown-days]"),E=u.querySelector("[data-countdown-hours]"),w=u.querySelector("[data-countdown-minutes]"),R=u.querySelector("[data-countdown-seconds]"),z=/^\d+$/.test(String(y))?Number(y):new Date(y).getTime();if(!y||Number.isNaN(z)){u.innerHTML=`
      ...
      `;return}const j=z-Date.now();if(j<=0){u.innerHTML=`
        ...
        `;const C=u.closest(".license-generated-card");C&&!C.dataset.expired&&(C.dataset.expired="true",je(C.dataset.licenseKey));return}const x=Math.floor(j/864e5),J=Math.floor(j%864e5/36e5),te=Math.floor(j%36e5/6e4),ae=Math.floor(j%6e4/1e3);g&&(g.textContent=String(x).padStart(2,"0")),E&&(E.textContent=String(J).padStart(2,"0")),w&&(w.textContent=String(te).padStart(2,"0")),R&&(R.textContent=String(ae).padStart(2,"0"))})}async function je(o){try{await fetch(`/api/licenses/${encodeURIComponent(o)}`,{method:"DELETE"});const u=document.querySelector(`[data-license-key="${CSS.escape(o)}"]`);u&&u.remove(),ee()}catch(u){console.error("Error eliminando licencia:",u)}}function Me(){clearInterval(Le),Y(),Le=setInterval(Y,1e3)}function Z(){return`
  ...
  `;try{const o=await fetch(`${O}/api/licenses`,{method:"GET",credentials:"include",headers:{Accept:"application/json"},cache:"no-store"}),u=await o.json();if(!o.ok||!u.success)throw new Error(u.message||"No se pudieron cargar las licencias.");const y=Array.isArray(u.licenses)?u.licenses:[];if(y.length===0){t.innerHTML=q(),ee();return}t.innerHTML=`
      ...
      ${y.map(g=>Pe(g,!1)).join("")}
    `,ee(),Me()}catch(o){console.error("Error cargando licencias:",o),t.innerHTML=`
      ${Z()}

      <div class="license-generation-error">
        ❌ ${o.message||"No se pudieron cargar las licencias."}
      </div>
    `,ee()}}}function Fe(){t?.querySelector(".server-dropdown-empty")?.remove(),t?.querySelector(".licenses-empty-state")?.remove(),t?.querySelector(".license-generation-error")?.remove()}function ee(){if(!t)return;const o=[...t.querySelectorAll(".license-generated-card")],u=o.length,y=o.filter(w=>w.querySelector(".license-status.active, .license-status.used")).length,g=o.filter(w=>w.querySelector(".license-status.available")).length,E=o.filter(w=>w.querySelector(".license-status.revoked")).length;i&&(i.textContent=String(u)),n&&(n.textContent=String(y)),c&&(c.textContent=String(g)),s&&(s.textContent=String(E))}function Ye(){t&&(t.querySelector(".license-generating-overlay")?.remove(),document.body.insertAdjacentHTML("beforeend",`
...
>
  ...
  >
    ...
    <b>↗</b>
      ...
      `,document.getElementById("publicDiscordSupport")?.addEventListener("click",()=>{window.open("https://discord.gg/ue5c56nyCr","_blank","noopener,noreferrer")})});document.getElementById("publicLogout")?.addEventListener("click",async()=>{$e();try{await fetch("/auth/discord/logout",{method:"POST",credentials:"include"})}catch(a){console.error("Error cerrando sesión:",a)}localStorage.removeItem("nebulaSelectedServerId"),window.location.replace("/auth/dashboard")});const Ii=document.getElementById("sidebar");document.getElementById("hamburger").addEventListener("click",()=>Ii.classList.toggle("open"));document.addEventListener("keydown",a=>{a.ctrlKey&&a.key.toLowerCase()==="k"&&(a.preventDefault(),document.querySelector(".search input").focus())});const Ke=document.getElementById("space");if(Ke){let t=function(){Ke.width=innerWidth*devicePixelRatio,Ke.height=innerHeight*devicePixelRatio,Ke.style.width=innerWidth+"px",Ke.style.height=innerHeight+"px",a.setTransform(devicePixelRatio,0,0,devicePixelRatio,0,0),e=Array.from({length:90},()=>({x:Math.random()*innerWidth,y:Math.random()*innerHeight,r:Math.random()*1.3+.2,a:Math.random()*.45+.05,s:Math.random()*.12+.02}))},i=function(){a.clearRect(0,0,innerWidth,innerHeight),e.forEach(n=>{n.y-=n.s,n.y<0&&(n.y=innerHeight),a.beginPath(),a.arc(n.x,n.y,n.r,0,Math.PI*2),a.fillStyle=`rgba(255,255,255,${n.a})`,a.fill()}),requestAnimationFrame(i)};var Vi=t,Ui=i;const a=Ke.getContext("2d");let e=[];t(),i(),addEventListener("resize",t)}const ki=document.querySelector(".main"),Ti=document.querySelector(".topbar"),_=document.createElement("div");_.id="pageContent";let ot=Ti.nextElementSibling;for(;ot;){const a=ot.nextElementSibling;_.appendChild(ot),ot=a}ki.appendChild(_);const ha=_.innerHTML;_.innerHTML=`
  <div class="dynamic-page">
...
`;Si();window.addEventListener("nebula:server-changed",async()=>{document.getElementById("licenseLogsChannel")&&await initializeLicenseSettings()});const Li={Servidores:{eyebrow:"SERVIDORES",title:"Tus servidores",description:"Seleccioná un servidor para administrar sus funciones, permisos y configuración.",action:"＋ Agregar servidor",type:"servers"},Invitaciones:{eyebrow:"SERVIDORES",title:"Invitaciones",description:"Creá, controlá y eliminá enlaces de invitación para tus comunidades.",action:"＋ Nueva invitación",type:"invitations"},Miembros:{eyebrow:"COMUNIDAD",title:"Miembros",description:"Buscá usuarios, revisá sus roles y consultá su actividad reciente.",action:"Exportar lista",type:"members"},Roles:{eyebrow:"COMUNIDAD",title:"Roles",description:"Organizá la jerarquía y los permisos de cada rol del servidor.",action:"＋ Crear rol",type:"roles"},Canales:{eyebrow:"ESTRUCTURA",title:"Canales",description:"Administrá canales de texto, voz, categorías y permisos.",action:"＋ Crear canal",type:"channels"},"Logs del servidor":{eyebrow:"REGISTROS",title:"Logs del servidor",description:"Consultá eventos, mensajes eliminados y movimientos administrativos.",action:"Exportar logs",type:"logs"},Comandos:{eyebrow:"COMANDOS",title:"Comandos",description:"Activá, desactivá y personalizá las funciones principales del bot.",action:"＋ Crear comando",type:"commands"},"Slash Commands":{eyebrow:"COMANDOS",title:"Slash Commands",description:"Gestioná los comandos que aparecen al escribir / dentro de Discord.",action:"Sincronizar",type:"slash"},Mensajes:{eyebrow:"MENSAJES",title:"Mensajes automáticos",description:"Diseñá mensajes de bienvenida, despedida, anuncios y verificaciones.",action:"＋ Nuevo mensaje",type:"messages"},"Auto Respuestas":{eyebrow:"AUTOMATIZACIÓN",title:"Auto Respuestas",description:"Configurá respuestas automáticas mediante palabras o frases clave.",action:"＋ Nueva respuesta",type:"responses"},Moderación:{eyebrow:"SEGURIDAD",title:"Centro de moderación",description:"Accedé rápidamente a todas las herramientas de control del servidor.",action:"Abrir historial",type:"moderation"},Advertencias:{eyebrow:"MODERACIÓN",title:"Advertencias",description:"Registrá faltas, motivos, responsables y vencimientos.",action:"＋ Advertir usuario",type:"warnings"},Baneos:{eyebrow:"MODERACIÓN",title:"Baneos",description:"Administrá usuarios bloqueados y revisá los motivos de cada sanción.",action:"＋ Banear usuario",type:"bans"},Muteos:{eyebrow:"MODERACIÓN",title:"Muteos",description:"Aplicá silenciamientos temporales y revisá los que siguen activos.",action:"＋ Mutear usuario",type:"mutes"},"Anti Raid":{eyebrow:"PROTECCIÓN",title:"Anti Raid",description:"Protegé el servidor contra ingresos masivos y comportamientos sospechosos.",action:"Activar protección",type:"antiraid"},"Auto Mod":{eyebrow:"PROTECCIÓN",title:"Auto Mod",description:"Filtrá spam, enlaces, menciones masivas y palabras prohibidas.",action:"Guardar reglas",type:"automod"},Logs:{eyebrow:"SISTEMA",title:"Logs generales",description:"Revisá todas las acciones realizadas por el bot y el dashboard.",action:"Descargar",type:"logs"},Auditoría:{eyebrow:"SISTEMA",title:"Auditoría",description:"Identificá quién realizó cada cambio dentro del panel.",action:"Filtrar actividad",type:"audit"},"Tareas programadas":{eyebrow:"AUTOMATIZACIÓN",title:"Tareas programadas",description:"Programá anuncios, limpiezas, backups y acciones automáticas.",action:"＋ Programar tarea",type:"tasks"},Backups:{eyebrow:"SISTEMA",title:"Backups",description:"Guardá y restaurá configuraciones del servidor de forma segura.",action:"Crear backup",type:"backups"},Webhooks:{eyebrow:"INTEGRACIONES",title:"Webhooks",description:"Conectá servicios externos para recibir y enviar información.",action:"＋ Crear webhook",type:"webhooks"},Configuración:{eyebrow:"AJUSTES",title:"Configuración general",description:"Definí la identidad, el estado y el comportamiento principal del bot.",action:"Guardar cambios",type:"settings"},Personalización:{eyebrow:"APARIENCIA",title:"Personalización",description:"Cambiá colores, textos, imágenes y estilo visual del panel.",action:"Aplicar diseño",type:"customize"},"Variables de entorno":{eyebrow:"DESARROLLO",title:"Variables de entorno",description:"Prepará las variables que usaremos luego en Render sin mostrar secretos reales.",action:"Guardar variables",type:"environment"},Tokens:{eyebrow:"SEGURIDAD",title:"Tokens",description:"Visualizá el estado de las credenciales sin exponer su contenido.",action:"Actualizar token",type:"tokens"},Permisos:{eyebrow:"SEGURIDAD",title:"Permisos del bot",description:"Revisá qué acciones puede realizar el bot dentro de Discord.",action:"Guardar permisos",type:"permissions"},Integraciones:{eyebrow:"CONEXIONES",title:"Integraciones",description:"Prepará conexiones con Discord, Render, GitHub y otros servicios.",action:"＋ Conectar servicio",type:"integrations"}};function Bi(a){return`
  ...
  `;try{const e=await fetch(`${O}/api/servers/${a}`),t=await e.json();if(!e.ok||!t.success)throw new Error(t.message||"No se pudo cargar el servidor");$i(t.data)}catch(e){console.error("Error cargando servidor:",e),_.innerHTML=`
      <div class="dynamic-page">
    ...
    `,document.getElementById("backToServers")?.addEventListener("click",()=>{Je(),window.history.replaceState({},"","/?view=servers")})}}function $i(a){const e=new Date(a.createdAt).toLocaleDateString("es-AR",{day:"2-digit",month:"2-digit",year:"numeric"}),t=a.name.split(" ").slice(0,2).map(s=>s.charAt(0)).join("").toUpperCase(),i=a.icon?`<img src="${a.icon}" alt="${a.name}">`:`<span>${t}</span>`,n=a.roles.length?a.roles.map(s=>`
  ...
  `,document.getElementById("backToServers")?.addEventListener("click",()=>{Je(),window.history.replaceState({},"","/?view=servers")}),document.querySelector('[data-tool="welcome"]')?.addEventListener("click",()=>{Mi(a)}),document.querySelector('[data-tool="verification"]')?.addEventListener("click",()=>{Ri(a)}),document.querySelectorAll('.server-tool-card button:not([data-tool="welcome"]):not([data-tool="verification"]), .server-small-action, .server-header-action').forEach(s=>{s.addEventListener("click",()=>{ne.querySelector("strong").textContent="Herramienta preparada",ne.querySelector("p").textContent="Esta función se conectará más adelante.",Ae()})})}async function Ri(a){_.innerHTML=`
    <div class="dynamic-page">
      <div class="servers-loading">
        <div class="loading-spinner"></div>
        <strong>
  ...
  `;try{const[e,t,i,n]=await Promise.all([fetch(`${O}/api/servers/${a.id}/text-channels`),fetch(`${O}/api/servers/${a.id}`),fetch(`${O}/api/servers/${a.id}/verification`),fetch(`${O}/api/bot/public-info`)]),c=await e.json(),s=await t.json(),p=await i.json(),m=await n.json();if(!e.ok||!c.success)throw new Error(c.message||"No se pudieron cargar los canales");if(!t.ok||!s.success)throw new Error(s.message||"No se pudieron cargar los roles");if(!i.ok||!p.success)throw new Error(p.message||"No se pudo cargar la configuración");Pi(a,c.data,s.data.roles,p.data,m.success?m.data:{})}catch(e){console.error("Error cargando verificación:",e),_.innerHTML=`
    ...
    `,document.getElementById("backToServerPanel")?.addEventListener("click",()=>{Re(a.id)})}}function le(a,e,t){return`
  ...
  `,M=({id:r,title:d,description:v,enabled:f,logChannelId:T="",channels:G="",showLogChannel:F=!0,icon:X="◈",extraContent:ie="",cardClass:P=""})=>{const se=String(G).replace(new RegExp(`value="${String(T)}"`,"g"),`value="${String(T)}" selected`);return`
    <article
...
`}[He,Be,A,$,V].forEach(r=>{r?.addEventListener("input",R)}),fe?.addEventListener("input",R),b?.addEventListener("input",()=>{const r=b.value.trim();w(r)&&fe&&(fe.value=r,R())}),R(),l("backToServerPanel")?.addEventListener("click",()=>{Re(a.id)}),document.querySelectorAll(".verify-tab").forEach(r=>{r.addEventListener("click",()=>{document.querySelectorAll(".verify-tab").forEach(d=>d.classList.remove("active")),document.querySelectorAll(".verify-tab-panel").forEach(d=>d.classList.remove("active")),r.classList.add("active"),document.querySelector(`[data-verify-panel="${r.dataset.verifyTab}"]`)?.classList.add("active")})}),l("verifyEnabled")?.addEventListener("change",r=>{const d=l("verifySystemStatus");d.textContent=r.target.checked?"ACTIVADO":"DESACTIVADO",d.classList.toggle("enabled",r.target.checked),d.classList.toggle("disabled",!r.target.checked)}),l("verifyLogs")?.addEventListener("change",r=>{l("verifyLogsSettings")?.classList.toggle("visible",!!r.target.value)});const z=(r,d)=>{const v=l(r),f=l(d);v?.addEventListener("input",()=>{f.value=v.value}),f?.addEventListener("input",()=>{/^#[0-9A-F]{6}$/i.test(f.value)&&(v.value=f.value)})};z("verifyEmbedColor","verifyEmbedColorText"),z("verifyLogColor","verifyLogColorText");function j(){return{splitImageUrl:document.getElementById("verifySplitImageUrl")?.value.trim()||"",splitImagePosition:document.getElementById("verifySplitImagePosition")?.value||"left",splitImageFit:document.getElementById("verifySplitImageFit")?.value||"cover",splitImageDarkness:Number(document.getElementById("verifySplitImageDarkness")?.value||45),splitImageWidth:Number(document.getElementById("verifySplitImageWidth")?.value||48),splitShowImage:document.getElementById("verifySplitShowImage")?.checked!==!1,splitShowDate:document.getElementById("verifySplitShowDate")?.checked!==!1,splitShowAccess:document.getElementById("verifySplitShowAccess")?.checked!==!1,cardDesign:document.querySelector('input[name="verificationCardDesign"]:checked')?.value||"classic",terminalTitle:document.getElementById("verifyTerminalTitle")?.value.trim()||"NEBULA SECURITY TERMINAL",terminalPrefix:document.getElementById("verifyTerminalPrefix")?.value.trim()||">",terminalStatusText:document.getElementById("verifyTerminalStatusText")?.value.trim()||"Sistema preparado",terminalBackgroundColor:document.getElementById("verifyTerminalBackgroundColorText")?.value||"#020703",terminalTextColor:document.getElementById("verifyTerminalTextColorText")?.value||"#d9ffe0",terminalAccentColor:document.getElementById("verifyTerminalAccentColorText")?.value||"#22c55e",terminalBorderColor:document.getElementById("verifyTerminalBorderColorText")?.value||"#14532d",terminalShowCursor:!!document.getElementById("verifyTerminalShowCursor")?.checked,terminalShowLines:!!document.getElementById("verifyTerminalShowLines")?.checked,terminalShowServer:!!document.getElementById("verifyTerminalShowServer")?.checked,terminalShowRole:!!document.getElementById("verifyTerminalShowRole")?.checked,terminalGlow:Number(document.getElementById("verifyTerminalGlow")?.value||25),terminalRadius:Number(document.getElementById("verifyTerminalRadius")?.value||10),pageName:document.getElementById("verifyPageName")?.value.trim()||"",pageDescription:document.getElementById("verifyPageDescription")?.value.trim()||"",logoUrl:document.getElementById("verifyLogoUrl")?.value.trim()||"",backgroundUrl:document.getElementById("verifyBackgroundUrl")?.value.trim()||"",primaryColor:document.getElementById("verifyPrimaryColorText")?.value||"#8b5cf6",secondaryColor:document.getElementById("verifySecondaryColorText")?.value||"#6d28d9",buttonColor:document.getElementById("verifyButtonColorText")?.value||"#7c3aed",textColor:document.getElementById("verifyTextColorText")?.value||"#ffffff",cardColor:document.getElementById("verifyCardColorText")?.value||"#0f0f1a",backgroundSolidColor:document.getElementById("verifyBackgroundSolidColorText")?.value||"#05050a",gradientStart:document.getElementById("verifyGradientStartText")?.value||"#05050a",gradientEnd:document.getElementById("verifyGradientEndText")?.value||"#160c2b",backgroundType:document.getElementById("verifyBackgroundType")?.value||"space",spaceBackground:!!document.getElementById("verifySpaceBackground")?.checked,animationsEnabled:!!document.getElementById("verifyAnimationsEnabled")?.checked,particlesEnabled:!!document.getElementById("verifyParticlesEnabled")?.checked,glowEnabled:!!document.getElementById("verifyGlowEnabled")?.checked,fadeEnabled:!!document.getElementById("verifyFadeEnabled")?.checked,hoverEnabled:!!document.getElementById("verifyHoverEnabled")?.checked,cursorGlowEnabled:!!document.getElementById("verifyCursorGlowEnabled")?.checked,buttonAnimationEnabled:!!document.getElementById("verifyButtonAnimationEnabled")?.checked,logoAnimationEnabled:!!document.getElementById("verifyLogoAnimationEnabled")?.checked,particleCount:Number(document.getElementById("verifyParticleCount")?.value||100),glowIntensity:Number(document.getElementById("verifyGlowIntensity")?.value||80),cardBlur:Number(document.getElementById("verifyCardBlur")?.value||18),cardOpacity:Number(document.getElementById("verifyCardOpacity")?.value||88),cardRadius:Number(document.getElementById("verifyCardRadius")?.value||24),cardShadow:Number(document.getElementById("verifyCardShadow")?.value||80),verifyButtonText:document.getElementById("verifyAppearanceButtonText")?.value.trim()||"Verificar con Discord",verifyButtonIcon:document.getElementById("verifyButtonIcon")?.value||"discord",verifyButtonSize:document.getElementById("verifyButtonSize")?.value||"large",verifyButtonRadius:Number(document.getElementById("verifyButtonRadius")?.value||14),verificationSound:!!document.getElementById("verifyVerificationSound")?.checked,errorSound:!!document.getElementById("verifyErrorSound")?.checked,openingSound:!!document.getElementById("verifyOpeningSound")?.checked,soundVolume:Number(document.getElementById("verifySoundVolume")?.value||50)}}function x(){const r=j(),d=document.getElementById("verifyAppearanceFrame");d?.contentWindow&&d.contentWindow.postMessage({type:"nebula-appearance-preview",appearance:r},window.location.origin)}document.getElementById("verifyAppearanceFrame")?.addEventListener("load",()=>{setTimeout(x,150)});function te(){const r=document.querySelector('input[name="verificationCardDesign"]:checked')?.value||"classic";document.querySelectorAll("[data-design-settings]").forEach(f=>{f.classList.toggle("active",f.dataset.designSettings===r)});const d={classic:["identity","palette","background","animations","card","button","sounds"],split:["identity","palette","background","button"],terminal:["identity","background","button"]},v=d[r]||d.classic;document.querySelectorAll("[data-appearance-section]").forEach(f=>{const T=f.dataset.appearanceSection;f.hidden=!v.includes(T)})}document.querySelectorAll('input[name="verificationCardDesign"]').forEach(r=>{r.addEventListener("change",()=>{te(),x()})}),te(),["verifyPageName","verifyPageDescription","verifyLogoUrl","verifyBackgroundUrl","verifyPrimaryColor","verifyPrimaryColorText","verifySecondaryColor","verifySecondaryColorText","verifyButtonColor","verifyButtonColorText","verifyTextColor","verifyTextColorText","verifyCardColor","verifyCardColorText","verifyBackgroundSolidColor","verifyBackgroundSolidColorText","verifyGradientStart","verifyGradientStartText","verifyGradientEnd","verifyGradientEndText","verifyBackgroundType","verifySpaceBackground","verifyParticlesEnabled","verifyAnimationsEnabled","verifyGlowEnabled","verifyFadeEnabled","verifyHoverEnabled","verifyCursorGlowEnabled","verifyButtonAnimationEnabled","verifyLogoAnimationEnabled","verifyParticleCount","verifyGlowIntensity","verifyCardBlur","verifyCardOpacity","verifyCardRadius","verifyCardShadow","verifyAppearanceButtonText","verifyButtonIcon","verifyButtonSize","verifyButtonRadius","verifyOpeningSound","verifyVerificationSound","verifyErrorSound","verifySoundVolume","verifySplitImageUrl","verifySplitImagePosition","verifySplitImageFit","verifySplitImageDarkness","verifySplitImageWidth","verifySplitShowImage","verifySplitShowDate","verifySplitShowAccess","verifyTerminalTitle","verifyTerminalPrefix","verifyTerminalStatusText","verifyTerminalBackgroundColor","verifyTerminalBackgroundColorText","verifyTerminalTextColor","verifyTerminalTextColorText","verifyTerminalAccentColor","verifyTerminalAccentColorText","verifyTerminalBorderColor","verifyTerminalBorderColorText","verifyTerminalShowCursor","verifyTerminalShowLines","verifyTerminalShowServer","verifyTerminalShowRole","verifyTerminalGlow","verifyTerminalRadius"].forEach(r=>{const d=document.getElementById(r);if(!d)return;const v=d.type==="checkbox"||d.tagName==="SELECT"?"change":"input";d.addEventListener(v,x)}),[["verifyPrimaryColor","verifyPrimaryColorText"],["verifySecondaryColor","verifySecondaryColorText"],["verifyButtonColor","verifyButtonColorText"],["verifyTextColor","verifyTextColorText"],["verifyCardColor","verifyCardColorText"],["verifyBackgroundSolidColor","verifyBackgroundSolidColorText"],["verifyGradientStart","verifyGradientStartText"],["verifyGradientEnd","verifyGradientEndText"],["verifyTerminalBackgroundColor","verifyTerminalBackgroundColorText"],["verifyTerminalTextColor","verifyTerminalTextColorText"],["verifyTerminalAccentColor","verifyTerminalAccentColorText"],["verifyTerminalBorderColor","verifyTerminalBorderColorText"]].forEach(([r,d])=>{const v=document.getElementById(r),f=document.getElementById(d);v?.addEventListener("input",()=>{f.value=v.value,x()}),f?.addEventListener("input",()=>{const T=f.value.trim();/^#[0-9a-f]{6}$/i.test(T)&&(v.value=T,x())})}),document.querySelectorAll(".appearance-range-control input[type='range']").forEach(r=>{const d=document.getElementById(`${r.id}Value`),v=()=>{d&&(d.textContent=`${r.value}${r.dataset.suffix||""}`),x()};r.addEventListener("input",v),v()}),x(),l("saveVerification")?.addEventListener("click",async()=>{const r=l("saveVerification"),d=Oe();if(!d.verificationChannelId){Y("Falta el canal","Seleccioná el canal de verificación.");return}if(!d.verifiedRoleId){Y("Falta el rol","Seleccioná el rol que recibirá el usuario.");return}r.disabled=!0,r.textContent="Guardando...";try{const v=await fetch(`${O}/api/servers/${a.id}/verification`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)}),f=await v.json();if(!v.ok||!f.success)throw new Error(f.message||"No se pudo guardar.");Y("Configuración guardada","Todos los cambios quedaron guardados.")}catch(v){Y("Error al guardar",v.message)}finally{r.disabled=!1,r.textContent="Guardar configuración"}}),l("sendVerificationPanel")?.addEventListener("click",async()=>{const r=l("sendVerificationPanel");l("saveVerification").click(),await new Promise(v=>setTimeout(v,700)),r.disabled=!0,r.textContent="Enviando...";try{const v=await fetch(`${O}/api/servers/${a.id}/verification/send`,{method:"POST"}),f=await v.json();if(!v.ok||!f.success)throw new Error(f.message||"No se pudo enviar el panel.");Y("Panel enviado","Revisá el canal de verificación en Discord.")}catch(v){Y("Error al enviar",v.message)}finally{r.disabled=!1,r.textContent="Enviar panel a Discord"}})}async function Mi(a){_.innerHTML=`
  ...
  `;try{const[e,t]=await Promise.all([fetch(`${O}/api/servers/${a.id}/text-channels`),fetch(`${O}/api/servers/${a.id}/welcome`)]),i=await e.json(),n=await t.json();if(!i.success||!n.success)throw new Error("No se pudo cargar la configuración");Oi(a,i.data,n.data)}catch(e){_.innerHTML=`
      <div class="dynamic-page">
        <div class="servers-error">
          <div class="servers-error-icon">!</div>
    ...
    `,document.getElementById("backToServerPanel")?.addEventListener("click",()=>{Re(a.id)})}}function Oi(a,e,t){const i=e.map(b=>`
    <option
      value="${b.id}"
...
>
  ...
  `;const n=document.getElementById("welcomeEnabled"),c=document.getElementById("welcomeChannel"),s=document.getElementById("welcomeTitle"),p=document.getElementById("welcomeMessage"),m=document.getElementById("welcomeColor"),B=document.getElementById("welcomeColorText"),N=document.getElementById("welcomeShowAvatar"),h=document.getElementById("welcomeDmEnabled"),M=document.getElementById("welcomeDmFields"),L=document.getElementById("welcomeDmTitle"),Ue=document.getElementById("welcomeDmMessage"),l=document.getElementById("welcomeDmColor"),pe=document.getElementById("welcomeDmColorText"),Ie=document.getElementById("welcomeDmShowAvatar"),oe=document.getElementById("welcomeDmPreviewSection"),W=Pt?.name||"Nebula Bot",D=U||{},ve=D.displayName||D.globalName||D.username||"Usuario",ke=D.avatar||"",Te=document.getElementById("welcomePreviewUserAvatar"),Ce=document.getElementById("welcomeDmPreviewAvatar"),qe=ke?`
      <img
        src="${ke}"
        alt="${ve}"
      >
    ...
    `;Te&&(Te.innerHTML=qe),Ce&&(Ce.innerHTML=qe);const Pe=Pt?.avatar||"",Le=document.getElementById("welcomePreviewBotName"),Y=document.getElementById("welcomeDmPreviewBotName"),je=document.getElementById("welcomePreviewBotAvatar"),Me=document.getElementById("welcomeDmPreviewBotAvatar");Le&&(Le.textContent=W),Y&&(Y.textContent=W);const Z=Pe?`
      <img
        src="${Pe}"
        alt="${W}"
      >
    `:W.charAt(0).toUpperCase();je&&(je.innerHTML=Z),Me&&(Me.innerHTML=Z);function q(){const b=rt(s.value,a),A=rt(p.value,a);document.getElementById("welcomePreviewTitle").textContent=b,document.getElementById("welcomePreviewMessage").textContent=A,document.getElementById("welcomePreviewEmbed").style.borderLeftColor=m.value;const $=document.getElementById("welcomePreviewUserAvatar");$&&($.style.display=N.checked?"block":"none");const V=rt(L.value,a),re=rt(Ue.value,a);document.getElementById("welcomeDmPreviewTitle").textContent=V,document.getElementById("welcomeDmPreviewMessage").textContent=re,document.getElementById("welcomeDmPreviewEmbed").style.borderLeftColor=l.value;const o=document.getElementById("welcomeDmPreviewAvatar");o&&(o.style.display=Ie.checked?"block":"none")}m.addEventListener("input",()=>{B.value=m.value,q()}),B.addEventListener("input",()=>{/^#[0-9A-F]{6}$/i.test(B.value)&&(m.value=B.value,q())}),s.addEventListener("input",q),p.addEventListener("input",q),h.addEventListener("change",()=>{M.classList.toggle("visible",h.checked),oe.classList.toggle("visible",h.checked),q()}),N.addEventListener("change",q),Ie.addEventListener("change",q),L.addEventListener("input",q),Ue.addEventListener("input",q),l.addEventListener("input",()=>{pe.value=l.value,q()}),pe.addEventListener("input",()=>{/^#[0-9A-F]{6}$/i.test(pe.value)&&(l.value=pe.value,q())}),document.querySelectorAll("[data-variable]").forEach(b=>{b.addEventListener("click",()=>{const A=b.dataset.variable,$=p.selectionStart,V=p.selectionEnd;p.value=p.value.slice(0,$)+A+p.value.slice(V),p.focus(),p.selectionStart=p.selectionEnd=$+A.length,q()})});const Q=document.getElementById("welcomeVariablesModal"),be=document.getElementById("welcomeVariablesSearch"),me=document.getElementById("welcomeVariablesModalList"),Oe=document.getElementById("welcomeVariablesModalEmpty"),Fe=document.querySelectorAll("[data-welcome-variable-category]");let ee="user";const Ye=[{key:"{user}",label:"Nombre corto del usuario",category:"user"},{key:"{mention}",label:"Mención del usuario",category:"user"},{key:"{username}",label:"Nombre de usuario",category:"user"},{key:"{displayname}",label:"Nombre visible",category:"user"},{key:"{userid}",label:"ID del usuario",category:"user"},{key:"{joindate}",label:"Fecha de ingreso",category:"user"},{key:"{server}",label:"Nombre del servidor",category:"server"},{key:"{serverid}",label:"ID del servidor",category:"server"},{key:"{members}",label:"Cantidad de miembros",category:"server"},{key:"{membercount}",label:"Cantidad de miembros",category:"server"},{key:"{date}",label:"Fecha actual",category:"date"},{key:"{time}",label:"Hora actual",category:"date"}];function ye(){if(!me)return;const b=String(be?.value||"").trim().toLowerCase(),A=Ye.filter(V=>{const re=V.category===ee,o=`${V.key} ${V.label}`.toLowerCase(),u=!b||o.includes(b);return re&&u});me.innerHTML=A.map(V=>`
          <button
            class="verify-variable-button"
        ...
        `).join("");const $=A.length>0;Oe&&(Oe.hidden=$,Oe.classList.toggle("visible",!$))}function He(b=null){Q&&(Q.classList.add("open"),Q.setAttribute("aria-hidden","false"),document.body.classList.add("variables-modal-open"),ee="user",be&&(be.value=""),Fe.forEach(A=>{A.classList.toggle("active",A.dataset.welcomeVariableCategory==="user")}),ye(),setTimeout(()=>{be?.focus()},100))}function Be(){Q&&(Q.classList.remove("open"),Q.setAttribute("aria-hidden","true"),document.body.classList.remove("variables-modal-open"))}document.querySelector("[data-open-welcome-variables]")?.addEventListener("click",b=>{b.preventDefault(),b.stopPropagation(),He(p)}),document.querySelectorAll("[data-close-welcome-variables]").forEach(b=>{b.addEventListener("click",A=>{A.preventDefault(),A.stopPropagation(),Be()})}),Fe.forEach(b=>{b.addEventListener("click",A=>{A.preventDefault(),A.stopPropagation(),ee=b.dataset.welcomeVariableCategory||"user",Fe.forEach($=>{$.classList.toggle("active",$===b)}),ye()})}),be?.addEventListener("input",ye),me?.addEventListener("click",async b=>{const A=b.target.closest("[data-copy-welcome-variable]");if(!A)return;b.preventDefault(),b.stopPropagation();const $=A.dataset.copyWelcomeVariable;if($)try{await navigator.clipboard.writeText($);const V=A.querySelector("b")?.textContent,re=A.querySelector("b");re&&(re.textContent="Copiado ✓",setTimeout(()=>{re.textContent=V||"Copiar"},1200))}catch(V){console.error("No se pudo copiar la variable:",V)}}),Q?.addEventListener("wheel",b=>{b.target.closest(".variables-categories, .variables-results")||b.preventDefault()},{passive:!1}),document.addEventListener("keydown",b=>{b.key==="Escape"&&Q?.classList.contains("open")&&Be()}),ye(),document.getElementById("backToServerPanel").addEventListener("click",()=>{Re(a.id)});async function fe(){const b=document.getElementById("saveWelcome");b.disabled=!0,b.textContent="Guardando...";try{const A=await fetch(`${O}/api/servers/${a.id}/welcome`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:n.checked,channelId:c.value,title:s.value,message:p.value,color:m.value,showAvatar:N.checked,dmEnabled:h.checked,dmTitle:L.value,dmMessage:Ue.value,dmColor:l.value,dmShowAvatar:Ie.checked})}),$=await A.json();if(!A.ok||!$.success)throw new Error($.message||"No se pudo guardar");return ne.querySelector("strong").textContent="Bienvenida guardada",ne.querySelector("p").textContent="Todos los cambios quedaron guardados.",Ae(),!0}catch(A){return ne.querySelector("strong").textContent="Error al guardar",ne.querySelector("p").textContent=A.message,Ae(),!1}finally{b.disabled=!1,b.textContent="Guardar configuración"}}document.getElementById("saveWelcome").addEventListener("click",fe),document.getElementById("testWelcome").addEventListener("click",async()=>{const b=document.getElementById("testWelcome");b.disabled=!0,b.textContent="Preparando prueba...";try{if(!await fe())return;b.textContent="Enviando...";const $=await fetch(`${O}/api/servers/${a.id}/welcome/test`,{method:"POST"}),V=await $.json();if(!$.ok||!V.success)throw new Error(V.message||"No se pudo enviar la prueba");ne.querySelector("strong").textContent="Prueba enviada",ne.querySelector("p").textContent="Revisá el canal de bienvenida en Discord.",Ae()}catch(A){ne.querySelector("strong").textContent="Error en la prueba",ne.querySelector("p").textContent=A.message,Ae()}finally{b.disabled=!1,b.textContent="Enviar mensaje de prueba"}}),q()}function rt(a,e){const t=new Date,i=U||{},n=i.username||"usuario",c=i.displayName||i.globalName||n,s=i.id||"000000000000000000";return String(a).replaceAll("{user}",n).replaceAll("{mention}",`@${n}`).replaceAll("{username}",n).replaceAll("{displayname}",c).replaceAll("{userid}",s).replaceAll("{server}",e.name).replaceAll("{serverid}",e.id).replaceAll("{members}",Number(e.members||0).toLocaleString("es-AR")).replaceAll("{membercount}",Number(e.members||0).toLocaleString("es-AR")).replaceAll("{joindate}",t.toLocaleDateString("es-AR")).replaceAll("{date}",t.toLocaleDateString("es-AR")).replaceAll("{time}",t.toLocaleTimeString("es-AR",{hour:"2-digit",minute:"2-digit"}))}function I(a){return String(a).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;")}function S(a){return I(a).replaceAll('"',"&quot;").replaceAll("'","&#039;")}Ot();
